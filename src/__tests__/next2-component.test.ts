...
describe("C.16.1 — keyed list, misma key objeto nuevo", () => {
    ...
    it("repeat clásico: misma key + objeto nuevo con datos distintos → remount (C.16.4)", () => {
        const items = signal([{ id: 1, name: "A" }]);
        const container = document.createElement("div");
        const cleanup = html`<ul>${() =>
            repeat(
        ...
        items.value = [{ id: 1, name: "B" }];
        // C.16.4: los bindings capturaron el item viejo → remount in-place.
        expect(container.textContent).toContain("B");
        cleanup();
    });

    it("repeat clásico: misma key + objeto nuevo shallow-equal conserva el nodo (C.16.4)", () => {
        const items = signal([{ id: 1, name: "A" }]);
        const container = document.createElement("div");
        const cleanup = html`<ul>${() =>
            repeat(
...
describe("A.21 — controllers (Lit-style)", () => {
    it("hooks completos: init→mounted→deactivate→activate→unmount", () => {
        const calls: string[] = [];
        class Ctrl {
            hostInit() { calls.push("init") } hostMounted() { calls.push("mounted") }
...
describe("B.12.7 — liveList (deltas)", () => {
    it("push/remove/set aplican deltas sin re-diff", async () => {
        ...
        const container = document.createElement("div");
        const cleanup = html`<ul>${() => list}</ul>`._render(container, null);
        await Promise.resolve();
        list.push({ id: 1, v: "a" });
        list.push({ id: 2, v: "b" });
        list.insert(1, { id: 3, v: "c" });
        await Promise.resolve();
        expect([...container.querySelectorAll("li")].map((l) => l.textContent)).toEqual(["a", "c", "b"]);
        const bNode = container.querySelectorAll("li")[2];
        list.set(2, { id: 2, v: "B" }); // actualiza in-place
        await Promise.resolve();
        expect(container.querySelectorAll("li")[2]).toBe(bNode); // mismo nodo
        expect(bNode.textContent).toBe("B");
        list.remove(3);
        await Promise.resolve();
        expect([...container.querySelectorAll("li")].map((l) => l.textContent)).toEqual(["a", "B"]);
        cleanup();
    });
    ...
    it("move() reordena preservando nodos", async () => {
        ...
        const container = document.createElement("div");
        html`<ul>${() => list}</ul>`._render(container, null);
        await Promise.resolve();
        list.push({ id: 1, v: "a" }); list.push({ id: 2, v: "b" }); list.push({ id: 3, v: "c" });
        await Promise.resolve();
        const cNode = container.querySelectorAll("li")[2];
        list.move(3, 0); // c al principio
        await Promise.resolve();
        const lis = container.querySelectorAll("li");
        expect([...lis].map((l) => l.textContent)).toEqual(["c", "a", "b"]);
        expect(lis[0]).toBe(cNode); // mismo nodo movido
    });
    ...
    it("clear() es bulk: borra la zona de una vez, no remove() por fila", async () => {
        ...
        const container = document.createElement("div");
        const cleanup = html`<ul>${() => list}</ul>`._render(container, null);
        for (let i = 0; i < 20; i++) list.push({ id: i, v: `v${i}` });
        await Promise.resolve();
        expect(container.querySelectorAll("li").length).toBe(20);

        // El path bulk hace un sibling walk O(n) — una removeChild por
        // nodo (markers + li), sin indexOf+splice+deltas por fila (O(n²))
        // ni Range.deleteContents (patológico en happy-dom).
        const spy = vi.spyOn(Node.prototype, "removeChild");
        list.clear();
        await Promise.resolve();
        expect(container.querySelectorAll("li").length).toBe(0);
        expect(spy.mock.calls.length).toBeLessThanOrEqual(10 * 20);
        expect(spy.mock.calls.length).toBeGreaterThan(0);
        spy.mockRestore();

        // La lista sigue viva tras clear.
        list.push({ id: 99, v: "post" });
        await Promise.resolve();
        expect(container.querySelectorAll("li").length).toBe(1);
        expect(container.querySelector("li")!.textContent).toBe("post");
        cleanup();
    });
});

describe("cobertura — hydration mismatch", () => {
    it("mismatch warn-remount: DOM se corrige y bindings quedan vivos", async () => {
        const n = signal(1);
        const C = defineComponent(() => html`<b>v:${() => n.value}</b>`);
        const container = document.createElement("div");
        // HTML "del servidor" que NO coincide con el descriptor
        container.innerHTML = `<div class="otro">contenido distinto</div>`;
        const warn = vi.spyOn(console, "warn").mockImplementation(() => { });
        const handle = hydrate(C({}), container);
        expect(warn).toHaveBeenCalled(); // mismatch detectado
        warn.mockRestore();
        await Promise.resolve();
        // remount produjo el DOM correcto
        expect(container.textContent).toContain("v:1");
        expect(container.querySelector("b")).toBeTruthy();
        // y la reactividad sigue viva tras el remount
        n.value = 42;
        await Promise.resolve();
    ...
    it("mismatch=throw lanza en vez de remontar", async () => {
        const n = signal(1);
        const C = defineComponent(() => html`<b>v:${() => n.value}</b>`);
        const container = document.createElement("div");
        // SSR correcto pero sin el marker de binding → mismatch en descriptor
        const htmlSsr = await renderToString(C({}), { markers: "hydration" });
        container.innerHTML = htmlSsr.replace(/<!--.*?-->/g, ""); // markers borrados
        expect(() => hydrate(C({}), container, { mismatch: "throw" })).toThrow();
    });
});

describe("cobertura — ciclos de memoria (ownership)", () => {
    it("dispose de effect padre libera computed hijo + todas las edges", async () => {
        const src = signal(0);
        let computedRef: any = null;
        const disposeParent = effect(() => {
            // computed creado dentro del effect → owned por él
            computedRef = computed(() => src.value * 2);
            computedRef.value;
            src.value;
        });
        expect(computedRef).toBeTruthy();
        // el effect es consumidor del computed → edge viva
        expect(computedRef.consumersHead).toBeTruthy();
        disposeParent();
        // tras dispose: el computed muere con el padre → sin edges
        expect(computedRef.consumersHead).toBeNull();
        expect(computedRef.sourcesHead).toBeNull();
        expect(src.consumersHead).toBeNull(); // effect y computed desuscritos
    });
    ...
    it("componente funcional: dispose corta edges del render effect", async () => {
        ...
        inst.mount(container, null);
        await Promise.resolve();
        expect(n.consumersHead).toBeTruthy();
        inst.unmount();
        expect(n.consumersHead).toBeNull(); // sin suscriptores residuales
    });
});

describe("cobertura — suspend bajo el kernel (A.16 + async)", () => {
    it("suspend dentro de funcional: fallback→resolved, unmount limpia", async () => {
        const { suspend } = await import("../elur/async");
        const n = signal(0);
        const C = defineComponent(() =>
            html`<section>
                ${suspend(
                () => Promise.resolve("datos-" + n.value),
                (data) => html`<p class="res">${data}</p>`,
                { fallback: html`<p class="fb">cargando</p>` },
            )}
            </section>`,
        );
        const inst = new (await import("../elur/component-kernel")).ComponentInstance(C({}));
        ...
        expect(container.querySelector(".res")!.textContent).toBe("datos-0"); // resuelto
        inst.unmount();
        expect(n.consumersHead).toBeNull(); // sin effects residuales
    });
});
