...
describe("unown — hijos dispuestos no se retienen", () => {
    it("dispose() des-registra al hijo de parent.children", () => {
        ...
        expect(outer.children!.length).toBe(10);
        for (const d of disposes) d();
        expect(outer.children!.length).toBe(0); // sin retención
    });
    ...
    it("dispose del padre con muchos hijos limpia todo", () => {
        ...
        outer.dispose();
        expect(outer.children).toBeNull();
    });
});
